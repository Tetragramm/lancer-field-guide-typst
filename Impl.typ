...
#let Section(name, img: none) = context {
  ...
  let PlaceImg() = {
    if (type(img) == array) {
      ...
      if (img.len() == 1) {
        ...
        set image(width: 50%, fit: "stretch")
        stack(dir: ltr, img.at(0), img.at(1))
      }
    }
  }

  place(top + center, hide()[= #name])
  align(top + left, box(width: 200%, height: 80%, PlaceImg()))
  ...
  pagebreak()
  align(top + right, box(width: 200%, height: 80%, PlaceImg()))
  place(center + bottom, box(
    width: 100%,
    height: 20%,
    fill: white,
...
#let FullPageImageFramed(img) = {
  page(
    background: {
      ...
      place(horizon + center, [#figure(img)#label(img.source)])
      DrawBackground()
    },
    footer: DrawPageNumbers(),
  )[]
...
#let FullPageImage(img) = {
  page(
    background: [
      #figure(img)#label(img.source)
    ],
    footer: none,
  )[]
}

#let FullColImage(img, refname: none) = {
  colbreak(weak: true)
  hide([Text #linebreak()])
  if (refname == none) {
    refname = "1"
  } else {
    refname = (..nums) => refname
  }
  context {
    set image(width: 4.25in, height: 11in, fit: "stretch")
    let ydiff = here().position().y.inches()
...
#let Infobox(clr: color, title: none, title_tech: none, body: none, flavor_text: none, drawing: none) = {
  box(
    grid(
      columns: 1,
      TitleBox(clr: clr, drawing: drawing, title, title_tech),
      if (body != none or flavor_text != none) {
...
#let AutoBox(activation: str, ..args) = {
  ...
  if (activation == "Free" or activation == "Quick" or activation == "Full" or activation == "Other") {
    ...
    assert(
      args.named().at("trigger", default: none) == none,
...
#let TraitBox(title: [], body, type: false) = {
  layout(size => {
    box(width: 100%, grid(
      columns: 1,
      cetz.canvas(length: size.width, {
        ...
        let special-rect(element, name: none) = group(
          ctx => {
            ...
            fill(red)
            stroke(none)
            line((0, 0), (w, 0), (w, 0.1 * h), (w - 1.463 * h, h), (0, h))
          },
          name: name,
        )
...
#let MountBox(title) = {
  box(
    layout(sz => cetz.canvas(length: sz.width, {
      ...
      let special-rect(element, name: none) = group(
        ctx => {
          let (_, n, s, e, w) = cetz.coordinate.resolve(
            ctx,
            element + ".north",
            element + ".south",
            element + ".east",
          ...
          fill(red)
          stroke(none)
          line((0, 0), (w, 0), (w, 0.8 * h), (w - 0.325 * h, h), (0, h))
        },
        name: name,
      )
      ...
      content(
        (0, 0),
        anchor: "south-west",
        box(inset: (left: 0.3em, right: 0.6em, y: 0.3em), text(
          fill: white,
          size: 10pt,
...
#let PlaceTalent(talent) = {
  ...
  assert(
    talent.ranks.at(0).at("description", default: none) != none,
    message: "talent.ranks.at(0) needs field: description, a content",
  )
  assert(
    talent.ranks.at(1).at("name", default: none) != none,
    message: "talent.ranks.at(1) needs field: name, a string",
  )
  assert(
    talent.ranks.at(1).at("description", default: none) != none,
    message: "talent.ranks.at(1) needs field: description, a content",
  )
  assert(
    talent.ranks.at(2).at("name", default: none) != none,
    message: "talent.ranks.at(2) needs field: name, a string",
  )
  assert(
    talent.ranks.at(2).at("description", default: none) != none,
  ...
  v(-1em)
  line(length: 100%, stroke: talentblue + 2pt)
  v(-1em)
  text(fill: talentblue, size: 16pt, CC.RankOne)
  text(size: 16pt)[*#upper(NoKeyword(talent.ranks.at(0).name))*]
  linebreak()
  talent.ranks.at(0).description
  v(-1em)
  line(length: 100%, stroke: talentblue + 2pt)
  v(-1em)
  text(fill: talentblue, size: 16pt, CC.RankTwo)
  text(size: 16pt)[*#upper(NoKeyword(talent.ranks.at(1).name))*]
  linebreak()
  talent.ranks.at(1).description
  v(-1em)
  line(length: 100%, stroke: talentblue + 2pt)
  v(-1em)
  text(fill: talentblue, size: 16pt, CC.RankThree)
  text(size: 16pt)[*#upper(NoKeyword(talent.ranks.at(2).name))*]
  linebreak()
  talent.ranks.at(2).description
}

#let FindFrame(lcp, name) = {
...
#let FindWeapon(lcp, name) = {
  lcp.weapons.find(weap => { upper(weap.name) == upper(name) })
}

#let FindSystem(lcp, name) = {
  lcp.systems.find(sys => { upper(sys.name) == upper(name) })
}

#let FindMod(lcp, name) = {
  lcp.mods.find(mod => { upper(mod.name) == upper(name) })
}
...
#let ParseWeapon(lcp, weap) = {
  if (type(weap) == str) {
    weap = FindWeapon(lcp, weap)
  }
  let tech = ()
  if ("mount" in weap) {
    tech.push([#weap.mount #weap.type])
  }
  if ("sp" in weap) {
    //Cost anything?
    tech.push([#weap.sp; SP])
  }
  if ("tags" in weap) {
    //Apply tags
    tech.push(ParseTags(lcp, weap.tags).join(", "))
  }
  tech = tech.join(",")
  if ("range" in weap) {
    // Apply Range(s)
  ...
  if ("damage" in weap) {
    ...
    for dmg in weap.damage {
      if ("val" in dmg) {
        dmglist.push(str(dmg.val) + CC.at(dmg.type))
      } else if ("damage" in dmg) {
        dmglist.push(dmg.damage.map(d => str(d)).join("/") + CC.at(dmg.type))
      }
    }
    if (dmglist.len() > 0) {
  ...
  if (utype == "DEPLOYABLE" or utype == "DRONE" or utype == "SHIELD" or utype == "SYSTEM") {
    if ("actions" in sys or "deployables" in sys) {
      if ("actions" in sys and sys.actions.at(0).at("name", default: 0) == 0) {
        ...
        if (litems.len() > 1) {
          eff.push(linebreak() + list(..litems))
    ...
    for aa in sys.at("actions", default: ()) {
      if ("name" in aa and aa.name != sys.name and "activation" in aa and aa.activation == "Reaction") {
        ...
        AutoBox(
          activation: aa.activation,
          title: aa.at("name", default: sys.name),
          title_tech: tech,
          trigger: aa.at("trigger", default: none),
          body: aa.detail,
        )
...
#let PlaceBlocks(frame, content_array, possible_colbreak_idx, figure) = {
  ...
  layout(
    sz => {
      ...
      for (idx, size) in sizes.enumerate() {
        ...
        if (idx != 0) {
          //If it's not the first element in the column, take off another 0.2em
          c1 = (c1 - 0.2em).to-absolute()
        }
        if (c1 < 0pt and c2idx == -1) {
          //If we've hit the end of the column, set the c2idx marker
          c2idx = idx
        }
        if (c1 < 0pt) {
          // If we've hit the end of the column, it's now on the 2nd column, so take that off.
          c2 = c2 - size.height
        }
        if (idx != c2idx) {
          //If it's not the first element in the column, take off another 0.2em
        ...
        if (c2 < 0pt and newpage_idx == -1) {
          //If we've hit the end of 2nd column, set the marker
          newpage_idx = idx
        }
  ...
  context {
    ...
    if (query(label(frame.name + "c_" + str(content_array.len() - 1))).len() == 0) {
      ...
      block(stroke: red + 2pt, outset: 8pt, inset: 0pt, breakable: true, block(
        stroke: red + 2pt,
        outset: 4pt,
        inset: 0pt,
        breakable: true,
...
#let ParseCoreSystem(lcp, core_system) = {
  let content_array = ()
  ...
  for pa in core_system.at("passive_actions", default: ()) {
    ...
    content_array.push(AutoBox(
      ...
      trigger: pa.at("trigger", default: none),
      body: AutoSymbolize(pa.detail),
    ))
  }
  ...
  if ("actions" in trait) {
    ...
    for ta in trait.actions {
      ...
      block(AutoBox(
        ...
        title: ta.name,
        trigger: ta.at("trigger", default: none),
        title_tech: AutoSymbolize(tech),
        body: AutoSymbolize(ta.detail),
      ))
    }
  } else {
    if ("description" in trait) {
...
#let PlaceFrame(frame, traits_blocks, mounts, core_system, background: none) = {
  ...
  let f = figure(placement: top, scope: "parent", [
    ...
    #context {
      ...
      place(top + left, float: false, dx: -12pt, dy: pos.y - 0.6in, box(
        stroke: red + 2pt,
        width: 100% + 24pt,
...
#let PlaceLicenseHeader(level, names) = {
  ...
  box(
    ...
    inset: 0.1em,
    outset: 0.5em,
    text(fill: white, hyphenate: false, NoKeyword(upper(strong([
...
#let PlaceLicenseFull(level, header_names, blocks) = {
  colbreak(weak: true)
  PlaceLicenseHeader(level, header_names)
  for b in blocks {
    block(b)
  }
}
...
#let LicenseAutomatic(lcp, license, level) = {
  ...
  PlaceLicenseFull(level, names, blocks)
}
...
#let PlaceNPC(frame, base_systems, optional_systems, background: none) = {
  ...
  let StatBlock(stats, tier) = {
    ...
    v(-1em)
    columns(2)[
      #set text(size: 10pt, font: "Ubuntu Mono")
      *Hull:* #stats.hull.at(tier)\
      *Agility:* #stats.agility.at(tier)
      #colbreak()
      *Systems:* #stats.systems.at(tier)\
      *Engineering:* #stats.systems.at(tier)
    ]
    v(-1em)
    text(size: 16pt)[*CORE STATS*]
    v(-2em)
    columns(2)[
      #set text(size: 10pt, font: "Ubuntu Mono")
      *HP:* #stats.hp.at(tier)\
      *Evasion:* #stats.evade.at(tier)\
      *Speed:* #stats.speed.at(tier)\
      *Heat Cap:* #stats.heatcap.at(tier)\
      *Sensors:* #stats.sensor.at(tier)
      #colbreak()
      *Armor:* #stats.armor.at(tier)\
      *E-Defense:* #stats.edef.at(tier)\
      *Size:* #stats.size.at(tier).join(" or ")\
      *Save Target:* #stats.save.at(tier)
    ]
  }
  ...
  let f = figure(placement: top, scope: "parent", [
    ...
    #align(center)[
      #box(width: 80%, [
        ...
        #text(fill: red, font: "Ubuntu Mono", [#v(-0.5em) #role.join("/")])
        #v(-1em)
        #align(left, text(font: "Ubuntu Mono", NoKeyword([
          #set par(spacing: 1.2em)
          #show regex("(<br>)+"): [#linebreak()#linebreak()]
    ...
    #NoKeyword()[
      #grid(
        ...
        align: left,
        inset: 0.5em,
        grid.cell(stroke: (top: red + 2pt))[
          #box(
            width: 100%,
            outset: (left: 0.5em + 1pt, top: 0.5em, right: 0.5em - 1pt),
            inset: (bottom: 0.5em),
            fill: red,
            text(fill: white, size: 12pt)[*TIER 1*],
          )
          #StatBlock(frame.stats, 1)
        ],
        grid.cell(stroke: (left: deepred + 2pt, top: deepred + 2pt))[
          #box(
            width: 100%,
            outset: (left: 0.5em + 1pt, top: 0.5em, right: 0.5em - 1pt),
            inset: (bottom: 0.5em),
            fill: deepred,
            text(fill: white, size: 12pt)[*TIER 2*],
          )
          #StatBlock(frame.stats, 2)
        ],
        grid.cell(stroke: (left: darkerred + 2pt, top: darkerred + 2pt))[
          #box(
            width: 100%,
            outset: (left: 0.5em + 1pt, top: 0.5em, right: 0.5em + 1pt),
            inset: (bottom: 0.5em),
            fill: darkerred,
            text(fill: white, size: 12pt)[*TIER 3*],
          )
          #StatBlock(frame.stats, 3)
        ],
      )
    ]
    #hide(line(length: 100%))
  ...
  f //Place it immediately
  text(size: 16pt)[*TACTICS*]
  linebreak()
  frame.info.tactics
  linebreak()
  linebreak()

  text(size: 16pt)[*BASE SYSTEMS*]
  linebreak()
  base_systems
  linebreak()
  linebreak()

  text(size: 16pt)[*OPTIONAL SYSTEMS*]
  linebreak()
  optional_systems
}

#let FindNPC(lcp, name) = {
  lcp.npc_classes.find(npc => { upper(npc.name) == upper(name) })
}

#let ParseNPC(lcp, frame_name) = {
  let frame = FindNPC(lcp, frame_name)
  let base_systems = for fid in frame.base_features {
    let feat = lcp.npc_features.find(f => f.id == fid)
    if feat.type == "Weapon" {
      ParseWeapon(lcp, feat)
    } else if (feat.type == "System") {
      ParseSystem(lcp, feat)
    } else if (feat.type == "Trait") {
      ParseTrait(feat)
    }
  }

  let optional_systems = for fid in frame.optional_features {
    let feat = lcp.npc_features.find(f => f.id == fid)
    if feat.type == "Weapon" {
      ParseWeapon(lcp, feat)
    } else if (feat.type == "System") {
  ...
  (frame, base_systems, optional_systems)
}

#let NPCAutomatic(lcp, frame_name, background: none) = {
  PlaceNPC(..ParseNPC(lcp, frame_name), background: background)
}
